...
if args.add:
    ...
    net = torch.load("/content/alexnet_decomposed_model.pth")
else:
    net = model
...
# Training
def train(epoch, decompose = True):
    """
    ...
    if not os.path.exists(folder_path + '/alexnet_ckpt.pth'):
...
#        print(f"Layer: {name}")
#        if 'bias' in name:
#            print(f"{name} values: {param.data}")
#        elif 'weight' in name:
#            if 'classifier' in name:
#                # Lấy giá trị của trọng số
#                weights_values = param.data.cpu().numpy()

              #Tạo tên file từ tên của lớp và chỉ số của trọng số
#                file_name = f"weights_{name}_"

                # Ghi giá trị trọng số vào file
#                with open(file_name, 'w') as file:
#                    for row in weights_values:
#                        file.write(" ".join(map(str, row)) + "\n")
#            elif 'features' in name:  # Kiểm tra xem có phải là lớp Conv2d không
                # Lấy giá trị của kernel (trọng số của lớp Conv2d)
#                kernel_values = param.data.cpu().numpy()

                # Tạo tên file từ tên của lớp và chỉ số của trọng số
#                file_name = f"kernel_{name}_"

                # Ghi giá trị kernel vào file
#                with open(file_name, 'w') as file:
#                    for kernel in kernel_values:
#                        file.write(" ".join(map(str, kernel)) + "\n")
...
#    label_file.close()
    ...
    state = torch.load(folder_path + '/alexnet_ckpt.pth')
...
if os.path.isdir('checkpoint'):
    ...
    weight_path = ('./checkpoint/alexnet_ckpt.pth')
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        checkpoint = torch.load('./checkpoint/alexnet_ckpt.pth')
        ...
        for i, key in enumerate(net.features._modules.keys()):
            ...
            if args.tucker:
                ...
                layer_to_decomp = layer_to_decomp_cp
            ...
            if isinstance(net.features._modules[key], torch.nn.modules.conv.Conv2d):
                ...
                if args.tucker:
                    ...
                    nparam, npd, ratio, decomposed = tucker_decomposition_conv_layer(conv_layer,
                                                                                     tucker_rank_selection_method,
                                                                                     target_ratio_)
                    count += nparam
                    count_d += npd
        ...
        #torch.save(net, modelpath + '/ckpt.pth')
        torch.save(net, modelpath + '/alexnet_ckpt.pth')
        if args.tucker:
            if count > 0: print("Total param reduction: " + str(count) + " ==> " + str(count_d) + "(X" + str(
                round(count / count_d, 2)) + ")")
        ...
        if args.fine_tune:
            if args.resume_d:
                ...
                checkpoint = torch.load('/content/checkpoint_decomp/alexnet_ckpt.pth')
                net.load_state_dict(checkpoint['net'])
                best_acc = checkpoint['best_acc']
